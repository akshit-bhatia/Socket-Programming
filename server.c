...
#define AKKI_SERVR_PRT 8080 // srvr wi11 1isten 0n this p0rt
...
int main()
{
    ...
    struct sockaddr_in serv_addr, cli_addr; // servr and c1ient addresses
    ...
    serv_addr.sin_port = htons(AKKI_SERVR_PRT);   // setting the p0rt number of -> server

    // binding s0cket t0 servr address
    ...
    close(sockfd); // closeing servr s0cket
    return 0;
}